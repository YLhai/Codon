...
import numpy as np
from .globBiasStat import globBiasStat
from .singleBiasStat import singleBiasStat
# 所有CDS合并的
...
def bias(input_cds,figname):
    ...
    glob_biasplot(globStat,figname+"_glob"+".pdf")
    single_biasplot(singleStat, figname+"_single"+".pdf")

def run(input_cds,figname,outputPath,figType="pdf"):

    globStat = outputPath+"globBiasStat.csv"
    singleStat = outputPath+"singleBiasStat.csv"
    globBiasStat(input_cds, globStat)
    singleBiasStat(input_cds, singleStat)
